...
export function EndnoteImport({
    ...
    const handleFileSelect = useCallback(async (selectedFile: File) => {
        ...
        try {
            ...
            // Count RIS/EndNote entries (each entry starts with TY -)
            const entries = (content.match(/^TY\s*-/gm) || []).length;
            const sizeKB = (selectedFile.size / 1024).toFixed(1);

            setPreviewStats({
                entries: entries || 1, // At least show 1 if we can't detect
                size: `${sizeKB} KB`,
            });
        } catch (err) {
